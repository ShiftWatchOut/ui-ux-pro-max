...
- 当前镜像：上游 `main`（版本与上游 commit 记录在 [`upstream.lock.json`](upstream.lock.json)，每次同步自动更新）
...
├── .github/workflows/
│   ├── sync-upstream.yml        # 每天定时 + 手动触发同步并自动提交
...
- 每天 `04:00`（Asia/Shanghai / `20:00 UTC`）自动运行
- 支持 **手动触发**（Actions → Sync upstream mirror → Run workflow），可指定上游 `ref`（分支或 tag，例如 `v2.13.0`）
- 流程：`git clone --depth 1` 上游 → `build_mirror.py build`（导出 + 改写 + 校验）→ 校验失败则不提交 → 有内容变化才 `git commit && git push`，提交信息带上游 `ref @ sha (v版本)`
- 内容没变化就不产生提交，CC Switch 侧也不会有更新提示

### 本地手动同步

```bash
# 拉上游 main、重建 skills/ 并校验（已有变更需自己 commit/push）
bash scripts/sync-upstream.sh

# 固定到某个 tag
bash scripts/sync-upstream.sh --ref v2.13.0
...
3. 所有文档中出现的脚本调用都满足上游强制的「skill 内相对路径」契约，且指向真实存在的文件（`scripts/<file>` / `../<skill>/scripts/<file>`）；
...
2. **核心 `SKILL.md` 的 11 处脚本路径改写**：
   ...
   ```
   这正是上游子技能一直使用、并被上游 `.github/workflows/check-asset-sync.yml` 与单测逐条强制的形式（"skill-relative paths resolve in every install context: plugin cache, project-level CLI install, CLI --global install, manual copy"）。基准目录就是本 `SKILL.md` 所在目录。
3. **排除** `**/tests/`、`__pycache__/`、`.coverage` 等非运行期文件。上游的 `tests/` 是仓库级回归测试，依赖仓库根目录布局（例如 `test_skill_script_paths.py` 靠仓库根标记定位），装到用户机器上无法运行。
4. **不含** `cli/`、`gallery/`、`stack/`、`projects/`、`screenshots/`、`docs/` 等开发目录。
5. 其余文件**字节级保持与上游一致**：`.gitattributes` 用 `* -text` 禁止检查时的换行转换，这样 `data/*.csv` 的 sha256 与上游相同，`scripts/validate_data.py` 里的快照校验才成立。
